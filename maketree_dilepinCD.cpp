...
//run with eg:
// clas12root -b 'maketree_dilepinCD.cpp("/cache/clas12/rg-b/production/recon/spring2019/torus-1/pass2/v0/dst/train/jpsi/jpsi_006334.hipo","config.dat","eedFS_test.root")'
//to cat output to log file:
//clas12root -b 'maketree_dilepinCD.cpp("/cache/clas12/rg-b/production/recon/spring2019/torus-1/pass2/v0/dst/train/jpsi/jpsi_006334.hipo","config.dat","eedFS_test.root")' > eedFS_test.log
//also works with wildcards in data path

void maketree_dilepinCD(string dataPath, string configFileName, string outLoc){
    ...
    for (Int_t i = 0; i < listfiles->GetEntries(); i++)
    {
        ...
        while (c12.next() == true)
        {
            ...
            auto electrons = c12.getByCharge(-1);
            auto deuterons = c12.getByID(45);
            auto positrons = c12.getByCharge(1);
            auto neutrals = c12.getByCharge(0);
            
            if(electrons.size()>0 && positrons.size() > deuterons.size() && deuterons.size()>0){ //positrons include deuterons
              int combis=0;
              for(int el_combis=0;el_combis<electrons.size();el_combis++){
                  for(int po_combis=0;po_combis<positrons.size();po_combis++){
                    ...
                    //make sure we don't use same part for
                    //deut and e+ candidate
                    if(po.P()!=deut.P()){
                      ...
                      int elStatus=electrons[el_combis]->par()->getStatus();
                      int poStatus=positrons[po_combis]->par()->getStatus();
                      ...
                      if(abs(elStatus)>=2000 && abs(elStatus)<4000){
                        //Trust EB PID in FD
                        if(abs(electrons[el_combis]->getPid())!=11){
                          writeOut=0;
                        } else{
                      ...
                      if(abs(poStatus)>=2000 && abs(poStatus)<4000){
                        //Trust EB PID in FD
                        if(abs(positrons[po_combis]->getPid())!=11){
                          writeOut=0;
                        } else{
                      ...
                      branchValues[getRow("elStatus", varNames,nVars)] = elStatus;
                      ...
                      branchValues[getRow("poStatus", varNames,nVars)] = poStatus;
                      ...
                      branchValues[getRow("UID", varNames,nVars)] = entries;
                       ...
                       if(elStatus >= 2000 && elStatus < 4000) {
                          branchValues[getRow("elTOFEnergy", varNames,nVars)] = electrons[el_combis]->sci(FTOF1B)->getEnergy();
                          branchValues[getRow("elTOFPath", varNames,nVars)] = electrons[el_combis]->sci(FTOF1B)->getPath();
                          branchValues[getRow("elTOFTime", varNames,nVars)] = electrons[el_combis]->sci(FTOF1B)->getTime();
                      } else {
                          branchValues[getRow("elTOFEnergy", varNames,nVars)] = electrons[el_combis]->sci(CTOF)->getEnergy();
                          branchValues[getRow("elTOFPath", varNames,nVars)] = electrons[el_combis]->sci(CTOF)->getPath();
                          branchValues[getRow("elTOFTime", varNames,nVars)] = electrons[el_combis]->sci(CTOF)->getTime();
                      }
                      branchValues[getRow("elBeta", varNames,nVars)] = electrons[el_combis]->par()->getBeta();
                      branchValues[getRow("elPID", varNames,nVars)] = electrons[el_combis]->getPid();

                      if(poStatus >= 2000 && poStatus < 4000) {
                          branchValues[getRow("poTOFEnergy", varNames,nVars)] = positrons[po_combis]->sci(FTOF1B)->getEnergy();
                          branchValues[getRow("poTOFPath", varNames,nVars)] = positrons[po_combis]->sci(FTOF1B)->getPath();
                          branchValues[getRow("poTOFTime", varNames,nVars)] = positrons[po_combis]->sci(FTOF1B)->getTime();
                      } else {
                          branchValues[getRow("poTOFEnergy", varNames,nVars)] = positrons[po_combis]->sci(CTOF)->getEnergy();
                          branchValues[getRow("poTOFPath", varNames,nVars)] = positrons[po_combis]->sci(CTOF)->getPath();
                          branchValues[getRow("poTOFTime", varNames,nVars)] = positrons[po_combis]->sci(CTOF)->getTime();
                      }
                      branchValues[getRow("poBeta", varNames,nVars)] = positrons[po_combis]->par()->getBeta();
                      branchValues[getRow("poPID", varNames,nVars)] = positrons[po_combis]->getPid();
...
void initNames(string* varNames){
    ...
    //to facilitate e+/e- in CD
    varNames[67]="elPID";
    varNames[68]="elTOFEnergy";
    varNames[69]="elTOFPath";
    varNames[70]="elTOFTime";
    varNames[71]="elBeta";

    varNames[72]="poPID";
    varNames[73]="poTOFEnergy";
    varNames[74]="poTOFPath";
    varNames[75]="poTOFTime";
    varNames[76]="poBeta";
